                    ...
                    |> P.andThen
                        (\( tipe, end ) ->
                            Space.chomp E.TSpace
                                |> P.andThen (\_ -> exprHelp start end tipe)
                        )
            )
                                            ...
                                            expression
                                                |> P.andThen
                                                    (\( returnType, newEnd ) ->
                                                        let
                                                            funcType =
                                                                Src.at start newEnd (TLambda tipe returnType)
                                                        in
                    ...
                    [ -- Type variable
                      Var.lower E.TStart
                        |> P.andThen
                            (\name ->
                                P.getPosition
                                    |> P.andThen (\end -> P.succeed ( Src.at start end (TVar name), end ))
                            )
                    , -- Type constructor
                ...
                P.getPosition
                    |> P.andThen
                        (\end ->
                            ...
                            in
                            Space.chomp E.TSpace
                                |> P.andThen (\_ -> chompTypeArgs start region upper [])
                        )
            )


chompTypeArgs : Position -> Region -> Var.Upper -> List Type -> Parser E.Problem ( Type, Position )
                            ...
                            (\_ ->
                                termNoApp
                                    |> P.andThen
                                        (\( arg, _ ) ->
                                            Space.chomp E.TSpace
                                                |> P.andThen (\_ -> chompTypeArgs start region upper (arg :: revArgs))
                                        )
                            )
                    ]
                    ( Src.at start end
                        (case upper of
                            Var.Unqualified name ->
                                TType region name (List.reverse revArgs)

                            Var.Qualified home name ->
                                TTypeQual region home name (List.reverse revArgs)
                        )
                    , end
                    )
            )
...
record start =
    P.inContext E.TRecord (P.symbol "{" E.TStart)
                        ...
                        [ -- Empty record
                          P.symbol "}" E.TRecordEnd
                            |> P.andThen
                                (\_ ->
                                    P.getPosition
                                        |> P.andThen (\end -> P.succeed ( Src.at start end (TRecord [] Nothing), end ))
                                )
                        , -- Record with fields or extension
                                                ...
                                                Space.chompAndCheckIndent E.TRecordSpace E.TRecordIndentColon
                                                    |> P.andThen
                                                        (\_ ->
                                                            let
                                                                nameStart =
                                                                    -- Calculate start position from end - name length
                                                                    { row = nameEnd.row
                                                                    , col = nameEnd.col - String.length name
                                                                    }

                                                                locatedName =
                                                                    Src.at nameStart nameEnd name
                                                            in
...
recordHelp : Position -> Maybe (Src.Located String) -> List ( Src.Located String, Type ) -> Parser E.Problem ( Type, Position )
            ...
            |> P.andThen
                (\_ ->
                    P.getPosition
                        |> P.andThen
                            (\end ->
                                P.succeed ( Src.at start end (TRecord (List.reverse fields) ext), end )
                            )
                )
        ]
                                                        ...
                                                        |> P.andThen
                                                            (\_ ->
                                                                Space.chompAndCheckIndent E.TRecordSpace E.TRecordIndentType
                                                                    ...
                                                                    |> P.andThen
                                                                        (\( fieldType, end ) ->
                                                                            P.succeed ( ( Src.at fieldStart fieldEnd name, fieldType ), end )
                                                                        )
                                                            )
                                                )
                                    )
...
tuple start =
    P.inContext E.TTuple (P.symbol "(" E.TStart)
                        ...
                        [ -- Unit
                          P.symbol ")" E.TTupleEnd
                            |> P.andThen
                                (\_ ->
                                    P.getPosition
                                        |> P.andThen (\end -> P.succeed ( Src.at start end TUnit, end ))
                                )
                            ...
                            |> P.andThen
                                (\( tipe, end ) ->
                                    Space.checkIndent end E.TTupleIndentEnd
                                        |> P.andThen (\_ -> tupleHelp start tipe [])
                                )
                        ]
                )
        )
