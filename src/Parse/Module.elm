...
parse source = P.run module_ source
                  ...
                  (\imports ->
                    chompDecls []
                      ...
                      P.andThen
                        (\decls ->
                          let
                            categorized = categorizeDecls [] [] [] [] [] decls
                          in
                          case maybeHeader of
                            Just ( name, exports ) ->
                              P.succeed
...
categorizeDecls :
  ...
  -> (List (Src.Located Src.Union))
     -> (List (Src.Located Src.Alias))
        -> (List (Src.Located Src.Infix)) -> (List Src.Port) -> (List Decl.Decl) -> CategorizedDecls
categorizeDecls values unions aliases binops ports decls =
  case decls of
    ...
    decl :: rest ->
      case decl of
        Decl.Value value    -> categorizeDecls (value :: values) unions aliases binops ports rest
        Decl.Union union    -> categorizeDecls values (union :: unions) aliases binops ports rest
        Decl.Alias alias_   -> categorizeDecls values unions (alias_ :: aliases) binops ports rest
        Decl.PortDecl port_ -> categorizeDecls values unions aliases binops (port_ :: ports) rest
chompModuleDocComment : Parser E.Problem (Maybe Src.DocComment)
chompModuleDocComment = P.oneOfWithFallback [ Space.chompDocComment E.ModuleSpace ] Nothing
                          ...
                          P.andThen
                            (\_ ->
                              Space.chompAndCheckIndent E.ModuleSpace E.ModuleProblem
                                ...
                                |>
                                P.andThen
                                          (\exports -> freshLine E.FreshLine |> P.andThen (\_ -> P.succeed (Just ( name, exports ))))
                            )
                      )
                )
...
chompImports imports =
  P.oneOfWithFallback
                      [ chompImport |> P.andThen (\import_ -> chompImports (import_ :: imports)) ]
                  ...
                  (\_ ->
                    P.oneOf
                      ...
                      [ freshLine E.ImportEnd
                          |>
                          P.andThen (\_ -> P.succeed { name = name, alias_ = Nothing, exposing_ = Explicit [] })
                      , chompAs name
                ...
                P.andThen
                  (\_ ->
                    P.oneOf
                      ...
                      [ freshLine E.ImportEnd
                          |>
                          P.andThen (\_ -> P.succeed { name = name, alias_ = Just alias_, exposing_ = Explicit [] })
        ...
        Space.chompAndCheckIndent E.ModuleSpace E.ImportIndentExposingList
          ...
          P.andThen
            (\exposed ->
              freshLine E.ImportEnd
                |>
                P.andThen (\_ -> P.succeed { name = name, alias_ = maybeAlias, exposing_ = exposed })
            )
      )
...
exposingHelp revExposed =
  P.oneOf
    ...
    , P.symbol ")" E.ExposingEnd |> P.andThen (\_ -> P.succeed (Explicit (List.reverse revExposed)))
    ]
      ...
      (\start ->
        P.oneOf
          ...
          [ Var.lower E.ExposingValue
              |>
              P.andThen
                        (\name -> P.getPosition |> P.andThen (\end -> P.succeed (Lower (Src.at start end name))))
          , P.symbol "(" E.ExposingValue
              |>
              P.andThen
                (\_ ->
                    ...
                    P.andThen
                      (\op ->
                        P.symbol ")" E.ExposingOperatorRightParen
                          |>
                          P.andThen
                                    (\_ -> P.getPosition |> P.andThen (\end -> P.succeed (Operator (Src.Region start end) op)))
                      )
                )
        ...
        P.andThen
          (\( decl, _ ) ->
            Space.chomp E.Declarations
              ...
              P.andThen
                (\_ ->
                  P.oneOfWithFallback [ freshLine E.DeclStart |> P.andThen (\_ -> chompDecls (decl :: decls)) ]
...
freshLine expecting =
  Space.chomp E.ModuleSpace |> P.andThen (\_ -> Space.checkFreshLine expecting)
