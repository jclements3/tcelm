    ...
    P.chompIf (\c -> c == '_') E.PStart
        |> P.andThen
            (\_ ->
                ...
                P.oneOfWithFallback
                    [ P.chompIf Char.isAlphaNum E.PStart
                        |> P.andThen (\_ -> P.fail (E.PWildcardNotVar "_" 1))
                    ]
                    ()
            )
...
record start =
    P.inContext E.PRecord (P.symbol "{" E.PStart)
                ...
                (\_ ->
                    P.oneOf E.PRecordOpen
                        ...
                        , P.symbol "}" E.PRecordEnd
                            |> P.andThen (\_ -> P.addEnd start (PRecord []))
                        ]
                )
        )
...
recordHelp start vars =
    P.oneOf E.PRecordEnd
        ...
        , P.symbol "}" E.PRecordEnd
            |> P.andThen (\_ -> P.addEnd start (PRecord (List.reverse vars)))
        ]
...
tuple start =
    P.inContext E.PTuple (P.symbol "(" E.PStart)
                            ...
                            |> P.andThen
                                (\( pattern, end ) ->
                                    Space.checkIndent end E.PTupleIndentEnd
                                        |> P.andThen (\_ -> tupleHelp start pattern [])
                                )
                        , P.symbol ")" E.PTupleEnd
                            |> P.andThen (\_ -> P.addEnd start PUnit)
                        ]
                )
        )
...
list start =
    P.inContext E.PList (P.symbol "[" E.PStart)
                            ...
                            |> P.andThen
                                (\( pattern, end ) ->
                                    Space.checkIndent end E.PListIndentEnd
                                        |> P.andThen (\_ -> listHelp start [ pattern ])
                                )
                        , P.symbol "]" E.PListEnd
                            |> P.andThen (\_ -> P.addEnd start (PList []))
                        ]
                )
        )
...
listHelp start patterns =
    P.oneOf E.PListEnd
        ...
        , P.symbol "]" E.PListEnd
            |> P.andThen (\_ -> P.addEnd start (PList (List.reverse patterns)))
        ]



-- EXPRESSION (pattern with cons and alias)
...
expression =
    P.getPosition
        |> P.andThen
            (\start ->
                exprPart
                    |> P.andThen (\ePart -> exprHelp start [] ePart)
            )


exprHelp : Position -> List Pattern -> ( Pattern, Position ) -> Parser E.Problem ( Pattern, Position )
                            ...
                            (\_ ->
                                Space.chompAndCheckIndent E.PSpace E.PIndentStart
                                    |> P.andThen
                                        (\_ ->
                                            exprPart
                                                |> P.andThen (\ePart -> exprHelp start (pattern :: revPatterns) ePart)
                                        )
                            )
                )
        , -- Alias
                                                                                ...
                                                                                Space.chomp E.PSpace
                                                                                    |> P.andThen
                                                                                        (\_ ->
                                                                                            let
                                                                                                alias =
                                                                                                    Src.at nameStart newEnd name

                                                                                                finalPattern =
                                                                                                    Src.at start newEnd (PAlias (cons revPatterns pattern) alias)
                                                                                            in
...
cons revPatterns lastPat =
    case revPatterns of
        ...
        prevPat :: rest ->
            cons rest (Src.merge prevPat lastPat (PCons prevPat lastPat))
                ...
                (\start ->
                    Var.foreignUpper E.PStart
                        |> P.andThen
                            (\upper ->
                                P.getPosition
                                    |> P.andThen (\end -> exprTermHelp (Region start end) upper start [])
                            )
                )
        , -- Simple term
          term
            |> P.andThen
                (\eterm ->
                    ...
                    in
                    Space.chomp E.PSpace
                        |> P.andThen (\_ -> P.succeed ( eterm, end ))
                )
        ]


exprTermHelp : Region -> Var.Upper -> Position -> List Pattern -> Parser E.Problem ( Pattern, Position )
                        ...
                        (\_ ->
                            P.oneOfWithFallback
                                ...
                                ( Src.at start end
                                    (case upper of
                                        Var.Unqualified name ->
                                            PCtor region name (List.reverse revArgs)

                                        Var.Qualified home name ->
                                            PCtorQual region home name (List.reverse revArgs)
                                    )
                                , end
                                )
                        )
