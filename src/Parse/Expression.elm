...
{-| Expression parsing for Elm.
This is the main expression parser, mirroring the Haskell compiler's Parse/Expression.hs
-}
                            ...
                            (\expr ->
                                P.getPosition
                                    |> P.andThen
                                        (\end ->
                                            Space.chomp E.Space
                                                |> P.andThen (\_ -> chompExprEnd start (State [] expr [] end))
                                        )
                            )
                    ]
            )
...
type alias State =
    { ops : List ( Expr, Src.Located String )
    , expr : Expr
    , args : List Expr
    , end : Position
    }
...
chompExprEnd start { ops, expr, args, end } =
    P.oneOfWithFallback
        [ -- argument (backtrackable so we can fall through if indentation check fails)
          P.backtrackable
        ...
        , -- operator (backtrackable so we can fall through if indentation check fails)
                                                                                    ...
                                                                                    Space.chomp E.Space
                                                                                        |> P.andThen
                                                                                            (\_ ->
                                                                                                let
                                                                                                    arg =
                                                                                                        Src.at opStart newEnd (Negate negatedExpr)
                                                                                                in
                                                                                                chompExprEnd start (State ops expr (arg :: args) newEnd)
                                                                                            )
                                                                                )
                                                                    ...
                                                                    |> P.andThen
                                                                        (\( newLast, newEnd ) ->
                                                                            let
                                                                                ...
                                                                                finalExpr =
                                                                                    Src.at start newEnd (Binops (List.reverse newOps) newLast)
                                                                            in
        ...
        (case ops of
            ...
            _ ->
                ( Src.at start end (Binops (List.reverse ops) (toCall expr args)), end )
        )
    ...
    P.oneOf E.Start
        [ P.symbol "-" E.Start
            |> P.andThen
                (\_ ->
                    term
                        |> P.andThen (\expr -> P.addEnd start (Negate expr))
                )
        , term
        ]
...
toCall func revArgs =
    case revArgs of
        ...
        lastArg :: _ ->
            Src.merge func lastArg (Call func (List.reverse revArgs))
...
stringLiteral start =
    String.string E.Start E.String
        |> P.andThen (\str -> P.addEnd start (Str str))


charLiteral : Position -> Parser E.Problem Expr
charLiteral start =
    String.character E.Start E.Char
        |> P.andThen (\chr -> P.addEnd start (Chr chr))
...
accessor start =
    P.symbol "." E.Dot
        |> P.andThen
            (\_ ->
                Var.lower E.Access
                    |> P.andThen (\field -> P.addEnd start (Accessor field))
            )
                                        ...
                                        (\field ->
                                            P.getPosition
                                                |> P.andThen
                                                    (\end ->
                                                        accessible start
                                                            (Src.at start end (Access expr (Src.at pos end field)))
                                                    )
                                        )
                            )
                )
...
list start =
    P.inContext E.List (P.symbol "[" E.Start)
                            ...
                            |> P.andThen
                                (\( entry, end ) ->
                                    Space.checkIndent end E.ListIndentEnd
                                        |> P.andThen (\_ -> chompListEnd start [ entry ])
                                )
                        , P.symbol "]" E.ListOpen
                            |> P.andThen (\_ -> P.addEnd start (List []))
                        ]
                )
        )
...
chompListEnd start entries =
    P.oneOf E.ListEnd
        ...
        , P.symbol "]" E.ListEnd
            |> P.andThen (\_ -> P.addEnd start (List (List.reverse entries)))
        ]
            ...
            |> P.andThen
                (\_ ->
                    P.oneOf E.TupleIndentExpr1
                                                                        ...
                                                                        |> P.andThen
                                                                            (\_ ->
                                                                                let
                                                                                    ...
                                                                                    expr =
                                                                                        Src.at exprStart negEnd (Negate negatedExpr)
                                                                                in
                                                                                chompExprEnd exprStart (State [] expr [] negEnd)
                                                                                    |> P.andThen
                                                                                        (\( entry, end ) ->
                                                                                            Space.checkIndent end E.TupleIndentEnd
                                                                                                |> P.andThen (\_ -> chompTupleEnd start entry [])
                                                                                        )
                                                                            )
                                                                )
                                                    )
                        ...
                        , -- Unit
                          P.symbol ")" E.TupleIndentExpr1
                            |> P.andThen (\_ -> P.addEnd start Unit)
                        , -- Regular tuple or parenthesized expression
                          expression
                            |> P.andThen
                                (\( entry, end ) ->
                                    Space.checkIndent end E.TupleIndentEnd
                ...
                (\_ ->
                    case List.reverse revExprs of
                        ...
                        secondExpr :: otherExprs ->
                            P.addEnd start (Tuple firstExpr secondExpr otherExprs)
                )
        ]
...
record start =
    P.inContext E.Record (P.symbol "{" E.Start)
        (Space.chompAndCheckIndent E.RecordSpace E.RecordIndentOpen
            |> P.andThen
                (\_ ->
                    P.oneOf E.RecordOpen
                        [ P.symbol "}" E.RecordOpen
                            |> P.andThen (\_ -> P.addEnd start (Record []))
                                                                    ...
                                                                    |> P.andThen
                                                                        (\firstField ->
                                                                            chompFields [ firstField ]
                                                                                |> P.andThen (\fields -> P.addEnd start (Update starter fields))
                                                                        )
                                                            )
                                                    , -- Regular record
                                                      P.symbol "=" E.RecordEquals
                                                                                            ...
                                                                                            |> P.andThen
                                                                                                (\_ ->
                                                                                                    chompFields [ ( starter, value ) ]
                                                                                                        |> P.andThen (\fields -> P.addEnd start (Record fields))
                                                                                                )
                                                                                    )
                                                                        )
                                                            )
...
chompFields fields =
    P.oneOf E.RecordEnd
        ...
        , P.symbol "}" E.RecordEnd
            |> P.andThen (\_ -> P.succeed (List.reverse fields))
        ]
                                                ...
                                                (\_ ->
                                                    expression
                                                        |> P.andThen
                                                            (\( value, end ) ->
                                                                Space.checkIndent end E.RecordIndentEnd
                                                                    |> P.andThen (\_ -> P.succeed ( key, value ))
                                                            )
                                                )
                                    )
                        )
                                                                                                            ...
                                                                                                            (\_ ->
                                                                                                                ...
                                                                                                                P.oneOf E.IfElseBranchStart
                                                                                                                    [ Keyword.if_ E.IfElseBranchStart
                                                                                                                        |> P.andThen (\_ -> chompIfEnd start newBranches)
                                                                                                                    , expression
                                                                                                                        |> P.andThen
                                                                                                                            (\( elseBranch, elseEnd ) ->
                                                                                                                                let
                                                                                                                                    ifExpr =
                                                                                                                                        Src.at start elseEnd (If (List.reverse newBranches) elseBranch)
                                                                                                                                in
...
lambda start =
    P.inContext E.Func (P.symbol "\\" E.Start)
                                                        ...
                                                        expression
                                                            |> P.andThen
                                                                (\( body, end ) ->
                                                                    let
                                                                        funcExpr =
                                                                            Src.at start end (Lambda (List.reverse revArgs) body)
                                                                    in
...
chompArgs revArgs =
    P.oneOf E.FuncArrow
        ...
        , P.symbol "->" E.FuncArrow
            |> P.andThen (\_ -> P.succeed revArgs)
        ]
                                                                                ...
                                                                                (\( firstBranch, firstEnd ) ->
                                                                                    chompCaseEnd [ firstBranch ] firstEnd
                                                                                        |> P.andThen
                                                                                            (\( branches, end ) ->
                                                                                                P.succeed
                                                                                                    ( Src.at start end (Case expr branches)
                                                                                                    , end
                                                                                                    )
                                                                                            )
                                                                                )
                                                                ...
                                                                (\_ ->
                                                                    expression
                                                                        |> P.andThen
                                                                            (\( body, end ) ->
                                                                                P.succeed
                                                                                    ( Src.at start end (Let defs body)
                                                                                    , end
                                                                                    )
                                                                            )
                                                                )
...
chompLetDefs : List (Src.Located Def) -> Position -> Parser E.Problem ( List (Src.Located Def), Position )
chompLetDefs revDefs end =
    -- Try to parse more definitions at the same indentation level
    P.oneOfWithFallback
        [ Space.checkAligned E.LetDefAlignment
            |> P.andThen
                (\_ ->
                    chompLetDef
                        |> P.andThen
                            (\( def, defEnd ) ->
                                chompLetDefs (def :: revDefs) defEnd
                            )
                )
        ]
...
chompDefArgsAndBody : Position -> Src.Located String -> Maybe Src.Type -> List Src.Pattern -> Parser E.Problem ( Src.Located Def, Position )
                            ...
                            (\_ ->
                                expression
                                    |> P.andThen
                                        (\( body, end ) ->
                                            P.succeed
                                                ( Src.at start end (Define name (List.reverse revArgs) body tipe)
                                                , end
                                                )
                                        )
                            )
                                                            ...
                                                            (\_ ->
                                                                expression
                                                                    |> P.andThen
                                                                        (\( expr, end ) ->
                                                                            P.succeed
                                                                                ( Src.at start end (Destruct pattern expr)
                                                                                , end
                                                                                )
                                                                        )
                                                            )
