...
generateRuntime _ =
    String.join "\n"
        ...
        , "    /* Handle strings (tag 2) */"
        , "    if (xs.tag == 2) return elm_str_append(xs, ys);"
        , "    /* Handle empty list (tag 100) */"
        ...
        , "}"
        , ""
        , "/* Index decoder - access array element by index */"
        , "static elm_value_t elm_Json_Decode_index_impl(elm_value_t idx, elm_value_t decoder, elm_value_t val) {"
        , "    if (val.tag != 905) return elm_err(elm_string(\"Expected array\"));"
        , "    elm_value_t items = *val.data.c;"
        , "    int64_t i = idx.data.i;"
        , "    while (i > 0 && items.tag == 101) {"
        , "        items = *items.next;"
        , "        i--;"
        , "    }"
        , "    if (i > 0 || items.tag != 101) return elm_err(elm_string(\"Array index out of bounds\"));"
        , "    return elm_apply1((elm_closure_t *)decoder.data.p, *items.data.c);"
        , "}"
        , ""
        , "static elm_value_t elm_Json_Decode_index(elm_value_t idx, elm_value_t decoder) {"
        , "    elm_closure_t *c = elm_alloc_closure();"
        , "    c->func = (void *)elm_Json_Decode_index_impl;"
        , "    c->arity = 3;"
        , "    c->applied = 2;"
        , "    c->args[0] = idx;"
        , "    c->args[1] = decoder;"
        ...
        , "}"
        , ""
        , "/* At decoder - nested field access */"
        , "static elm_value_t elm_Json_Decode_at_impl(elm_value_t path, elm_value_t decoder, elm_value_t val) {"
        , "    elm_value_t curr = val;"
        , "    elm_value_t pathList = path;"
        , "    while (pathList.tag == 101) {"
        , "        elm_value_t key = *pathList.data.c;"
        , "        if (curr.tag != 906) return elm_err(elm_string(\"Expected object\"));"
        , "        elm_value_t pairs = *curr.data.c;"
        , "        elm_value_t found = elm_nothing();"
        , "        while (pairs.tag == 101) {"
        , "            elm_value_t *kvPair = (elm_value_t *)pairs.data.c;"
        , "            elm_value_t k = kvPair[0];"
        , "            if (k.tag == 903 && strcmp(k.data.s, key.data.s) == 0) {"
        , "                found = elm_just(kvPair[1]);"
        , "                break;"
        , "            }"
        , "            pairs = *pairs.next;"
        , "        }"
        , "        if (found.tag == 200) return elm_err(elm_string(\"Field not found\"));"
        , "        curr = *found.data.c;"
        , "        pathList = *pathList.next;"
        , "    }"
        , "    return elm_apply1((elm_closure_t *)decoder.data.p, curr);"
        , "}"
        , ""
        , "static elm_value_t elm_Json_Decode_at(elm_value_t path, elm_value_t decoder) {"
        , "    elm_closure_t *c = elm_alloc_closure();"
        , "    c->func = (void *)elm_Json_Decode_at_impl;"
        , "    c->arity = 3;"
        , "    c->applied = 2;"
        , "    c->args[0] = path;"
        , "    c->args[1] = decoder;"
        ...
        , "}"
        , ""
        , "/* OneOf decoder - try multiple decoders */"
        , "static elm_value_t elm_Json_Decode_oneOf_impl(elm_value_t decoders, elm_value_t val) {"
        , "    elm_value_t curr = decoders;"
        , "    while (curr.tag == 101) {"
        , "        elm_value_t decoder = *curr.data.c;"
        , "        elm_value_t result = elm_apply1((elm_closure_t *)decoder.data.p, val);"
        , "        if (result.tag == 301) return result; /* Ok */"
        , "        curr = *curr.next;"
        , "    }"
        , "    return elm_err(elm_string(\"No decoder matched\"));"
        , "}"
        , ""
        , "static elm_value_t elm_Json_Decode_oneOf(elm_value_t decoders) {"
        , "    elm_closure_t *c = elm_alloc_closure();"
        , "    c->func = (void *)elm_Json_Decode_oneOf_impl;"
        , "    c->arity = 2;"
        , "    c->applied = 1;"
        , "    c->args[0] = decoders;"
        , "    return (elm_value_t){ .tag = 400, .data.p = c, .next = NULL };"
        , "}"
        , ""
        , "/* Maybe decoder - wraps result in Maybe instead of failing */"
        , "static elm_value_t elm_Json_Decode_maybe_impl(elm_value_t decoder, elm_value_t val) {"
        , "    elm_value_t result = elm_apply1((elm_closure_t *)decoder.data.p, val);"
        , "    if (result.tag == 300) return elm_ok(elm_nothing()); /* Err -> Ok Nothing */"
        , "    return elm_ok(elm_just(*result.data.c)); /* Ok x -> Ok (Just x) */"
        , "}"
        , ""
        , "static elm_value_t elm_Json_Decode_maybe(elm_value_t decoder) {"
        , "    elm_closure_t *c = elm_alloc_closure();"
        , "    c->func = (void *)elm_Json_Decode_maybe_impl;"
        , "    c->arity = 2;"
        , "    c->applied = 1;"
        , "    c->args[0] = decoder;"
        , "    return (elm_value_t){ .tag = 400, .data.p = c, .next = NULL };"
        , "}"
        , ""
        , "/* Value decoder - returns raw JSON value */"
        , "static elm_value_t elm_Json_Decode_value_impl(elm_value_t val) {"
        , "    return elm_ok(val);"
        , "}"
        , ""
        , "static elm_value_t elm_Json_Decode_value(void) {"
        , "    elm_closure_t *c = elm_alloc_closure();"
        , "    c->func = (void *)elm_Json_Decode_value_impl;"
        , "    c->arity = 1;"
        , "    c->applied = 0;"
        , "    return (elm_value_t){ .tag = 400, .data.p = c, .next = NULL };"
        , "}"
...
isBuiltin name =
    List.member name
        ...
        , "Json.Decode.index", "Json.Decode.at", "Json.Decode.oneOf"
