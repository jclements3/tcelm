...
type alias Account = { id : Int, name : String, accountType : AccountType, balance : Int }
type alias TransactionEntry = { accountId : Int, amount : Int }
type alias Transaction = { id : Int, description : String, entries : List TransactionEntry }
type alias Ledger =
...
emptyLedger : Ledger
emptyLedger =
  { accounts = Dict.empty, transactions = [], nextAccountId = 1, nextTransactionId = 1 }
...
createAccount name accountType ledger =
  let
    ...
    account = { id = accountId, name = name, accountType = accountType, balance = 0 }
...
postTransaction description entries ledger =
  do
    ...
    let
      ...
      transaction = { id = txnId, description = description, entries = entries }
    Ok
      { ledger
...
getBalance accountId ledger =
  let
    accounts = ledger.accounts
  in
  Dict.get accountId accounts |> Maybe.map getAccountBalance
main : Int
main =
  let
    ...
    entries =
      ...
      , { accountId = revenueAccount.id, amount = -10000 } ]

    result = postTransaction "Initial sale" entries ledger2
  in
  case result of
    Ok finalLedger -> getBalance cashAccount.id finalLedger |> Maybe.withDefault 0
    Err _          -> -1
